...
class PhaseHandler:
    """Class to handle phase state transitions and valid actions."""

    @classmethod
    def get_valid_actions(cls, round_state: Round, player_idx: int) -> list[Action]:
    ...
    @classmethod
    def handle_step(cls, round_state: Round, action: Action) -> list[GameEvent]:
        ...
        if round_state.phase == RoundPhase.SETUP:
            return cls._handle_setup_phase(round_state, player_idx, action)
        if round_state.phase == RoundPhase.DRAW:
            return cls._handle_draw_phase(round_state, player_idx, action)
        if round_state.phase == RoundPhase.ACTION:
            return cls._handle_action_phase(round_state, player_idx, action)
        if round_state.phase == RoundPhase.FLIP:
            return cls._handle_flip_phase(round_state, player_idx, action)

        return []

    @classmethod
    def _handle_setup_phase(
        cls, round_state: Round, player_idx: int, action: Action
    ) -> list[GameEvent]:
    ...
    def _handle_action_phase(
        ...
        if isinstance(action, ActionSwapCard):
            ...
            cls._end_turn(round_state, events)
    ...
    @classmethod
    def _end_turn(cls, round_state: Round, events: list[GameEvent]) -> None:
