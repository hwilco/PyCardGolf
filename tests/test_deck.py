...
def random_cards(num_cards: int):
    return [Card(random.randint(1, 13), random.choice(list(Suit)), random.choice(['red', 'blue'])) for
...
class TestCardStack(unittest.TestCase):
    ...
    def test_add_card_stack(self):
        card_stack = CardStack(cards=self.cards_5.copy())
    ...
    def test_num_cards(self):
        card_stack = CardStack(self.cards_5)
    ...
    def test_shuffle(self):
        ...
        for seed in [random.randint(0, sys.maxsize) for _ in range(10)]:
            card_stack = CardStack(cards=cards.copy(), seed=seed)
            r.seed(seed)
    ...
    def test_eq(self):
        self.assertEqual(CardStack(seed=1), CardStack(seed=1))
        self.assertEqual(CardStack(seed=1, cards=self.cards_5.copy()), CardStack(seed=1, cards=self.cards_5.copy()))

        self.assertNotEqual(CardStack(seed=1), CardStack(seed=2))
        self.assertNotEqual(CardStack(seed=1, cards=self.cards_5.copy()), CardStack(seed=2, cards=self.cards_5.copy()))
        self.assertNotEqual(CardStack(seed=1, cards=self.cards_5.copy()), CardStack(seed=1))
        self.assertNotEqual(CardStack(seed=1, cards=self.cards_5.copy()), CardStack(seed=1, cards=self.cards_5
                                                                                    .copy()[:2]))

    def test_str(self):
        self.assertEqual(str(CardStack(seed=1)), "Stack of 0 cards")
        card_stack = CardStack(cards=self.cards_5.copy())
    ...
    def test_repr(self):
        self.assertEqual(repr(CardStack(seed=1)), "CardStack(cards=[], seed=1)")
        card_stack = CardStack(cards=self.cards_5.copy(), seed=1)
...
class TestDeck(unittest.TestCase):
    def setUp(self):
        self.red_deck = Deck(color='red', seed=1)

    def test_cards(self):
        self.assertTrue(all(Card(rank, suit, 'red') in self.red_deck._cards
    ...
    def test_add_card_stack_wrong_suit(self):
        ...
        other_card_stack = CardStack(cards=[Card(copied_card.rank, copied_card.suit, 'blue')])
        self.assertRaises(ValueError, self.red_deck.add_card_stack, other_card_stack)
        self.assertEqual(other_card_stack.num_cards, 1)
